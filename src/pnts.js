...
export function buildFeatureTableMetadata({
  ...
  const bounds = toEcef(Bounds.min(nativeBounds)).concat(
    toEcef(Bounds.max(nativeBounds))
  );
...
export function buildFeatureTableString({ ept, options = {}, bounds, points }) {
  return (
    buildFeatureTableMetadata({ ept, options, bounds, points })
    |> JSON.stringify
    |> ((v) => Util.padRight(v, 8))
  );
}
...
export function calculateSizes({ ept, options = {}, bounds, points }) {
  const featureTableString = buildFeatureTableString({
    ept,
    options,
    bounds,
    points,
...
export function buildXyz({
  ...
  for (let o = 0; o < output.length; o += Constants.pntsXyzSize) {
    toEcef(extractors.map((v) => v(buffer, point))).forEach((v, i) =>
      output.writeFloatLE(v - mid[i], o + i * 4)
    );
    ++point;
  }
}
...
export function buildRgbFromIntensity({ output, ept, points, buffer, sizes }) {
  ...
  for (let point = 0; point < points; ++point) {
    intensities[point] = Math.round(
      ((intensities[point] - min) / (max - min)) * 255
    );
  }
...
export function buildRgb({ output, ept, points, buffer, sizes, options }) {
  ...
  const extractors = ["Red", "Green", "Blue"].map((v) =>
    Binary.getExtractor(ept.schema, v)
  );
